...
interface Props {
  ...
  loading?: boolean;
  disabled?: boolean;
  variant?: 'plain' | 'primary' | 'secondary' | 'primary-outline' | 'secondary-outline';
}
...
const MainBtn: React.FC<MainBtnProps> = ({
  ...
  const classes = cs('p-3 rounded shadow', {
    [`${className}`]: className,
    'button--plain': variant === 'plain',
    'button--primary bg-primary text-white': variant === 'primary',
    'button--secondary bg-secondary text-white': variant === 'secondary',
    'button--primaryOutline': variant === 'primary-outline',
    'button--secondaryOutline': variant === 'secondary-outline',
    'button--fullWidth w-full': fullWidth,
    'button--disabled opacity-60': disabled,
    'button--loading opacity-60': loading
  });
  return (
